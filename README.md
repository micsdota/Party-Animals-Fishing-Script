...
#### GUI版本 (推荐)
1.  **运行GUI版本**:
    *   双击 `better_fisher(GUI).py` 文件。
    *   程序会启动一个现代化的图形界面，提供更直观的操作体验。

2.  **GUI界面功能**:
    *   **分栏布局**: 界面分为三个主要区域 - 统计信息、状态控制和运行日志
    *   **实时统计**: 左侧面板实时显示当前运行统计和历史数据对比
    *   **状态控制**: 中间面板提供开始/暂停按钮和统计功能控制
    *   **日志显示**: 右侧面板实时显示运行日志，支持不同颜色标记不同类型信息
    *   **快捷键支持**: 支持与控制台版本相同的快捷键操作 (Ctrl+L, Ctrl+K等)
